...
<div class="header">
        ...
        ?>
            <form method="post">
                <input type="text"  class="text_login" name="acc" placeholder="Tài Khoản">
                <input type="password" name="pass" class="text_login" placeholder="Mật Khẩu">
...
<div class="panel">
  ...
  <fieldset>
    ...
    <div class="list_prod">
        <br><center>Gợi ý cho bạn !</center>
...
where begin_time <= (select (current_date)) and end_time >=(select (current_date))";  
            ...
            //chinh sua gia san pham
            /*$query = "update drink set d_cost= d_cost *".$percent." /100  ;";  
            $result = pg_query($query); */

            // ket thuc chinh sua

            $query = "select * from drink where category = 'Bia'";  
            $result = pg_query($query);  
            for($i=0;$i<=8;$i++)
            {
                $row = pg_fetch_assoc($result);
