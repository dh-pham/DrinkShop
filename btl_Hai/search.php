                    ...
                    if (isset($_POST['xnorder']))
            {
                ...
                <form method="post" >
                    ...
                    <div class ="msgTa" id="msgTa" style="background-color: white;" >
                        <?php
                        //$db = pg_connect("host=localhost port=5432 dbname=postgres user=postgres password=Milubamhanc");
                        $query ="select cmt_content from comment_msg where acc_name = '".$_SESSION["sacc_name"]."' and d_name='znul' order by comment_id desc ;";
                        $result = pg_query($query);  
                        $row = pg_fetch_assoc($result);
...
<div class="panel">
  ...
  <fieldset>
    ...
    <form method="post" action="order.php">
        ...
        <br> <center>Kết quả tìm kiếm</center> <br>
        ...
        <?php 
            ...
            $answer = $_POST['items'];  
            if ($answer == "pname") {          
                $query = "select * from drink where d_name like '".$_POST['searchguest']."%'";     
            }
            else {
                $query = "select * from drink where producer like '".$_POST['searchguest']."%'";     
            }  
            ...
            $query = "select * from drink where d_name like '".$_POST['search11']."%'"; 
            //$db = pg_connect("host=localhost port=5432 dbname=postgres user=postgres password=Milubamhanc");  
